...
/* SAMS connector — register with the conductor (presence UP + commands DOWN). Local-first; Phase 4 → cloud URL. Fails silent if unreachable. */
(function(){ try { window.SAMS_BOOT={id:'bull',name:'Bill the Bull',url:'http://127.0.0.1:4319'}; var s=document.createElement('script'); s.src='assets/sams-boot.js'; s.async=true; (document.head||document.documentElement).appendChild(s); } catch(e){} })();
...
async function renderChart(host,sym,fallback){
  ...
  function draw(){const cw=host.querySelector('#cw');let m=sliceRange(rows,cur);if(!m||!m.length){m=(fallback||[]).map(v=>({d:'',v}));}if(!m.length){cw.innerHTML='<div class="dimn">No price history.</div>';return;}cw.innerHTML=curve(m,760,200,m[m.length-1].v>=m[0].v?'#0c7245':'#a8283a');const lab=$('#rangelbl');if(lab){const ch=(m[m.length-1].v/m[0].v-1)*100;lab.innerHTML=`<span class="${cls(ch)}">${pc(ch)}</span> ${cur}`;}}
...
/* Per-position news badge (from the refresh ritual via status.json positions[].news): 📰 count + latest headline; tooltip lists cached headlines (≤1h TTL). */
function newsAge(ts){const t=Date.parse(ts||'');if(!isFinite(t))return '';const m=Math.max(0,Math.round((Date.now()-t)/60000));if(m<60)return m+'m';const h=Math.round(m/60);return h<48?h+'h':Math.round(h/24)+'d';}
function newsBadge(p){const n=p.news||[];if(!n.length)return '<span class="dim">—</span>';const tip=n.map(h=>'• '+h.headline+(h.source?' — '+h.source:'')+(h.publishedAt?' ('+newsAge(h.publishedAt)+')':'')).join('\n').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;');const latest=esc((n[0].headline||'').slice(0,36))+((n[0].headline||'').length>36?'…':'');return `<span title="${tip}">📰 ${n.length} <span class="dim">${latest}</span></span>`;}
function posTable(P){return tableSort(P.map(x=>Object.assign({newsCount:(x.news||[]).length},x)),[{k:'t',h:'Sym'},{k:'qty',h:'Qty'},{k:'price',h:'Last'},{k:'mktVal',h:'Value'},{k:'unrealPct',h:'P&L%'},{k:'dayPct',h:'Day'},{k:'thesis',h:'Thesis'},{k:'newsCount',h:'News'}],
  p=>`<tr onclick="go('ticker/${p.t}')"><td>${p.t}${p.lev?'<span class="tag">'+p.lev+'</span>':''}</td><td>${p.qty}</td><td>$${(+p.price).toFixed(2)}</td><td>${usd(p.mktVal)}</td><td class="${cls(p.unrealPct)}">${pc(p.unrealPct)}</td><td class="${cls(p.dayPct)}">${pc(p.dayPct)}</td><td>${thesisBadge(p)}</td><td>${newsBadge(p)}</td></tr>`,'mktVal');}
...
function pgRisk(){const r=DATA.risk||{},c=DATA.caps||{};
  ...
  <div class="grid2">
    ...
    ${card('Exposure & heat','heat + cash govern how many names fit — no slot count',meter('Portfolio heat (aggregate open risk)',r.heatUsedPct!=null?r.heatUsedPct:0,c.portfolioHeat||15)+`<div class="dimn" style="margin-top:6px">Gross exposure <b>${r.grossExposure||0}%</b> · largest single <b>${r.largestPos||0}%</b> (cap ${c.maxPosition||40}%) · sector <b>${r.sectorConc||0}%</b> (cap ${c.sectorCap||30}%)</div>`)}
  </div>
  ${card('Active guardrails','aggressive paper profile','<ul class="rules">'+
     ...
     `<li>Portfolio heat (aggregate open risk): <b>${c.portfolioHeat||15}%</b> cap</li>`+
...
function renderTicker(t){const d=DATA;const p=(d.positions||[]).find(x=>x.t===t)||{};const j=(d.journal||[]).filter(x=>x.t===t);const T=(d.tickers||{})[t]||{};
  ...
  ${(p.news&&p.news.length)?card('Recent headlines','cached ≤1h · existing feed',p.news.map(h=>`<div class="dimn" style="padding:6px 0;border-bottom:1px solid var(--line)">${h.url?`<a href="${esc(h.url)}" target="_blank" rel="noopener">${esc(h.headline)}</a>`:esc(h.headline)} <span class="dim">${esc(h.source||'')}${h.publishedAt?' · '+esc(newsAge(h.publishedAt))+' ago':''}</span></div>`).join('')):''}
...
const SITES=[
  ...
  {id:'bull', name:'Bull', tag:'Trading console',    url:'https://bull.dimsylaisolutions.com',mark:'◆',color:'#bfa765'},
