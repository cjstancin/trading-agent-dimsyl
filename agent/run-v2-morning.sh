#!/usr/bin/env bash
# Bill v2 — morning ritual (see systemd/v2/bill2-morning.timer for the schedule).
...
cd "$(dirname "$0")"
mkdir -p logs
LOG=logs/v2-morning.log
echo "---- $(date -Is) v2 morning ----" >> "$LOG"
npm run v2:morning >> "$LOG" 2>&1 || true
