...
Active profile: ${rules.name} — risk ~${rules.riskPerTradePct}%/trade, max ${Math.round((rules.maxPositionPct ?? 0.4) * 100)}%/position, max ${rules.maxOpen} open, price ≥ $${rules.minPrice}, ~${rules.trailPercent}% trailing stop.
...
ACCOUNT SIZE matters — positions use WHOLE shares (no fractional), so a name is only tradeable if ≥1 share fits the ${Math.round((rules.maxPositionPct ?? 0.2) * 100)}% position cap (≈ $${Math.round((rules.maxPositionPct ?? 0.2) * (Number(acct.equity) || 0))}). On a small account FAVOR liquid quality names + sector ETFs priced below that; skip names too expensive to take a real position in (an unaffordable pick is a wasted slot).

Use web search for movers, breakouts, catalysts, and the market regime (risk-on/off). Pick AT MOST ${rules.maxOpen} **quality, liquid** US names you'd be comfortable holding **1 week to ~5 years** — large/mid-cap real companies (real revenue) or liquid broad/sector ETFs (SPY/QQQ/XLK/SMH-type). **EXCLUDE entirely: penny stocks (price < $${rules.minPrice}), leveraged/inverse ETFs (NO SOXL/TQQQ/3x), crypto, meme/pump names, illiquid or pre-revenue lottery tickets.** Aggressive here = concentration + conviction in GOOD names, not gambles — quality over quantity, fine to pick fewer or none. For EACH idea give: ticker · setup (momentum breakout / pullback-in-uptrend / catalyst / quality-trend) · one-line thesis · entry zone · protective stop.
