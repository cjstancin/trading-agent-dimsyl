...
const trackRecord = renderTrackRecordBlock(ledger, minTagTrades());
// Per-symbol memory: Bill's own record on every name he's previously traded (capped to the most recently
// traded), so the watchlist is built KNOWING his history on names he knows. History-not-edge framing lives
// in the block itself; advisory only. "" until the first reconciled close.
